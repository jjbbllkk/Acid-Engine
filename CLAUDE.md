...
This is a VCV Rack 2.x plugin implementing a TB-303 acid synth emulation called "Acid Engine".

### Key Files

- `src/plugin.cpp` / `src/plugin.hpp` - Plugin registration and global instance
- `src/AcidEngine.cpp` - Main module: parameters, inputs/outputs, audio processing via `Open303` engine
- `src/open303/` - The Open303 synthesizer engine (rosic library), providing TB-303 emulation

### Module Structure

The `AcidEngine` module wraps the `rosic::Open303` synth engine:
...
The SVG panel is at `res/AcidEngine.svg`. Widget positions in `AcidEngineWidget` use `mm2px()` coordinates.
